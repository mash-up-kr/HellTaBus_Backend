...
export class generate1636767002484 implements MigrationInterface {
    name = 'generate1636767002484'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`hell_ta_bus\`.\`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nickname\` varchar(255) NULL, \`email\` varchar(255) NOT NULL, \`googleAccount\` varchar(255) NULL, \`gender\` enum ('MALE', 'FEMALE') NULL, \`age\` int UNSIGNED NULL, \`height\` int UNSIGNED NULL, \`weight\` int UNSIGNED NULL, \`splitType\` enum ('FULL_BODY_WORKOUT', 'SPLIT_3_DAY_WORKOUT', 'SPLIT_5_DAY_WORKOUT') NULL, \`audioCoach\` enum ('SCARY', 'COMFORTABLE', 'KID') NULL, \`speed\` enum ('SLOW', 'NORMAL', 'FAST') NULL, \`explanation\` tinyint NULL DEFAULT '0', \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), UNIQUE INDEX \`IDX_ec0b5b4045a9b6f42ccb2f6acc\` (\`googleAccount\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`hell_ta_bus\`.\`exercise-history\` (\`id\` int NOT NULL AUTO_INCREMENT, \`startTime\` datetime NOT NULL, \`finishTime\` datetime NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`userId\` int NULL, \`exerciseId\` int NULL, \`feedbackId\` int NOT NULL, UNIQUE INDEX \`REL_c6bcfe9a7cc70fa5f3f1cb242f\` (\`feedbackId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`hell_ta_bus\`.\`exercise-history\` DROP FOREIGN KEY \`FK_c6bcfe9a7cc70fa5f3f1cb242f5\``);
        ...
        await queryRunner.query(`DROP INDEX \`REL_c6bcfe9a7cc70fa5f3f1cb242f\` ON \`hell_ta_bus\`.\`exercise-history\``);
